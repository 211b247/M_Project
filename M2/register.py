...
class Register:
    def __init__(self, root):
        ...
        self.txt_contact.bind('<KeyRelease>', self.validate_contact)
    ...
    def validate_name(self, event):
        """
        Validates the input in first name and last name fields.
        """
        entry = event.widget
        value = entry.get()
        if not value.isalpha() and value != "":
            entry.delete(len(value) - 1, END)
            messagebox.showerror("Error", "Only alphabets are allowed in the name fields.", parent=self.root)

    def validate_contact(self, event):
        """
        Validates the input in contact field to allow only numeric characters.
        """
        entry = event.widget
        value = entry.get()
        if not value.isdigit() and value != "":
            entry.delete(len(value) - 1, END)
            messagebox.showerror("Error", "Only numbers are allowed in the contact field.", parent=self.root)
        elif len(value) > 10:
            entry.delete(len(value) - 1, END)
            messagebox.showerror("Error", "Contact number should contain exactly 10 digits.", parent=self.root)
